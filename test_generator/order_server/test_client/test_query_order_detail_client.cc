 ...
 *    #			###   #		  #				#				 #   #		#					####
 *
 * test_query_order_detail_client.cc
...
void test_client() {
  ...
  OrderService_Stub stub(&channel);

  tinyrpc::TinyPbRpcController rpc_controller;
  rpc_controller.SetTimeout(5000);

  ::queryOrderDetailReq rpc_req;
  ::queryOrderDetailRsp rpc_res;

  std::cout << "Send to tinyrpc server " << addr->toString() << ", requeset body: " << rpc_req.ShortDebugString() << std::endl;
  stub.query_order_detail(&rpc_controller, &rpc_req, &rpc_res, NULL);
